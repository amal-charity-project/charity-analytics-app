...
# دالة إصلاح النصوص العربية في الرسوم
...
# 2. توليد البيانات وتثبيتها سحابياً
...
# --- إدخل بيانات مستفيدين ➕ ---
    ...
    with tab_excel:
        ...
        if uploaded_file_b is not None:
            uploaded_df_b = pd.read_csv(uploaded_file_b) if uploaded_file_b.name.endswith('.csv') else pd.read_excel(uploaded_file_b)
            ...
            if st.button("🚀 دمج وتحديث ملفات المستفيدين فوراً"):
                ...
                st.success("🎉 تم رفع ودمج السجلات بنجاح وتحديث النظام!")
                st.rerun()
...
# --- التحليلات التنبؤية (AI) ---
    ...
    if len(df_donors) >= 5:
        ...
        st.warning("⚠️ يتطلب النظام وجود 5 متبرعين على الأقل.")

# --- معمل الإحصاء التعليمي ---
